...
namespace RVTR.Account.DataContext.Repositories
{
  ...
  public class AccountRepository : Repository<AccountModel, AccountDTO>
  {
    ...
    //                                                                      .FirstOrDefaultAsync();
    public override async Task<AccountModel> SelectAsync(int id)
    {
      var searchedAccount = await _db
        .Where(x => x.Id == id)
        .Include(x => x.Address)
        .Include(x => x.Profiles)
        .Include(x => x.Payments)
        .FirstOrDefaultAsync();
      return _mapper.Map<AccountModel>(searchedAccount);
    }
  }
